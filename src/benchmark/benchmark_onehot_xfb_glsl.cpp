...
bool benchmark::benchmark_onehot_xfb_glsl::checkSupport() noexcept {
	return benchmark_onehot_xfb::checkSupport() && glsl::checkSupport();
}

benchmark::benchmark_onehot_xfb_glsl::benchmark_onehot_xfb_glsl() {
	gl::GLuint vertShader, fragShader;

	vertShader = common::loadCompileShader(gl::GL_VERTEX_SHADER, "benchmark_xfb_v_glsl");
...
std::pair<double, benchmark::benchmark_base::doRMSE_return_t> benchmark::benchmark_onehot_xfb_glsl::drawFrame() {
	generatePoints();
	clearResults();
