...
const uint MAX_INVOCATIONS = 500000;
...
vec3 SolDipField(vec3 pos) {
    if(pos.z>SOL_MIN_Z) {
        ...
        int segID = segment_cache.SolSegCache[gl_PrimitiveIDIn];
        ...
        if(segID >=0 && IsInsideSol(segID, rphiz)) {
            vec3 brphiz = EvalSol(segID, rphiz);
            segment_cache.SolSegCache[gl_PrimitiveIDIn] = segID;
            return CyltoCartCylB(rphiz, brphiz);
        }
    }

    int segID = segment_cache.DipSegCache[gl_PrimitiveIDIn];
    ...
    if(segID >= 0 && IsInsideDip(segID, pos)) {
        segment_cache.DipSegCache[gl_PrimitiveIDIn] = segID;
        return EvalDip(segID, pos);
    }
